/**
 * 管理画面サイドバー最下段: 社員情報（/api/user）を主表示とし、LINE 名は補助的にのみ使う。
 * js/admin-app.js の updateUserInfo から buildSidebarUserInfoHtml / applyMenuVisibilityFromUser を利用する。
 *
 * 別ツール単体利用: AdminSidebarUserInfo.mount({ apiBaseUrl, fetchWithAuth, applyMenuVisibility: true })
 */
(function (global) {
    'use strict';
    ...
    /**
     * @param {string|null|undefined} lineDisplayName - 生の LINE 表示名（未登録時の補助表示用）。null で非表示。
     * @param {object|null} systemUser - /api/user の JSON
     * @param {string|null} fetchError - 通信エラー文言
     * @param {string} [logoutButtonId='logout-btn']
     */
