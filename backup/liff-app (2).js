...
function addTaskEntry(task = null) {
    ...
    majorInput.addEventListener('click', async () => {
        try {
            const selectedValue = await showSelectionModal('業務種別を選択', categoryAOptions, majorInput);
            majorInput.value = selectedValue;
            // 大分類が変更されたら、小分類をクリアする
    ...
    minorInput.addEventListener('click', async () => {
        try {
            ...
            minorInput.value = selectedObj.label || selectedObj; 
            if (typeof selectedObj === 'object') {
...
async function setupCategoryDatalists() {
    ...
    try {
        ...
        if (responseB.ok) {
            ...
            // ラベル（数字を含むテキスト）の降順にソート
            categoryBOptions.sort((a, b) => {
...
function showSelectionModal(title, options, inputElement) {
    return new Promise((resolve, reject) => {
        ...
        modal.style.paddingTop = '20px'; // 上部の余白を詰める
        ...
        optionsContainer.innerHTML = options.map((opt, index) => {
            ...
            if (typeof opt === 'object' && opt.label) {
                ...
                // clientは最大8文字
                const clientShort = opt.client.length > 8 ? opt.client.substring(0, 8) + '...' : opt.client;
            ...
            return `<button type="button" class="selection-option" data-index="${index}">${displayHtml}</button>`;
        }).join('');
...
async function initializeCalendarScreen(calendarContainer) {
    ...
    // 2. 【重要】当日分の勤務時間をリアルタイムで取得し、カレンダーデータを上書き＆ステータスを再計算する
    const todayStr = toLocalDateString(new Date());
    ...
    if (calendarStatuses[todayStr]) {
        ...
        if (workTimeResult.success) {
            ...
            const reportedMinutes = calendarStatuses[todayStr].reported_minutes || 0;

            if (jobcanMinutes > 0 && reportedMinutes === 0) {
                calendarStatuses[todayStr].status = 'pending'; // 未入力
            } else if (jobcanMinutes > 0 && jobcanMinutes === reportedMinutes) {
                calendarStatuses[todayStr].status = 'completed'; // 完了
            } else if (jobcanMinutes > 0 && reportedMinutes > 0 && jobcanMinutes !== reportedMinutes) {
                calendarStatuses[todayStr].status = 'inconsistent'; // 不一致
            }
        }
    }
...
function initManagerUserSettingsPage() {
    ...
    async function fetchUnmappedEmployees() {
        ...
        try {
            ...
            const unmappedEmployees = await response.json();

            if (unmappedEmployees.length === 0) {
                resultsContainer.innerHTML = `<p class="sync-message success">未登録のユーザーは見つかりませんでした。全員システムに登録済みです。</p>`;
            } else {
